...
import wandb
from datas.SimpleLearningAugment import SimpleLearningAugment
...
class LearnDiversifyEnv(object):
    def __init__(
        ...
        self.convertor = (
            DDP(
              SimpleLearningAugment(
                  policy=yaml["LAA"]["augmentation_policy"],
                  C=yaml["LAA"]["C"],
                  H=yaml["LAA"]["H"],
                  W=yaml["LAA"]["W"],
                  p=yaml["LAA"]["p"],
    ...
    def run_one_train_batch_size(self, batch_idx, indexs, input, target):
        ...
        # TODO: Learning to diversify
        with torch.no_grad():
            inputs_max, target_temp = self.convertor.module(input, target)
        ...
        if mask.sum().item()>0:
            ...
            change_tensor_to_image(inputs_max[mask].squeeze(),"image",f"_{self.epoch}")
        ...
        if not self.only_satge_one:
            ...
            inputs_max, target_temp = self.convertor(input, target)
            ...
            distance3 = - F.kl_div(aug_logits.log_softmax(1), target, reduction="batchmean")
            task_loss = distance2 + distance1 * 0.2 + distance3 * 0.04 # left 0.8 right `.1
