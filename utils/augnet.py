...
class SmallImageAugNet(nn.Module):
    def __init__(self, img_size=224):
        ...
        self.noise_lv = nn.Parameter(torch.zeros(1))
        self.shift_var = nn.Parameter(torch.empty(3, img_size - 2, img_size - 2))
        nn.init.normal_(self.shift_var, 1, 0.1)
        self.shift_mean = nn.Parameter(torch.zeros(3, img_size - 2, img_size - 2))
        nn.init.normal_(self.shift_mean, 0, 0.1)

        self.shift_var2 = nn.Parameter(torch.empty(3, img_size - 4, img_size - 4))
        nn.init.normal_(self.shift_var2, 1, 0.1)
        self.shift_mean2 = nn.Parameter(torch.zeros(3, img_size - 4, img_size - 4))
        nn.init.normal_(self.shift_mean2, 0, 0.1)

        self.shift_var3 = nn.Parameter(torch.empty(3, img_size - 6, img_size - 6))
        nn.init.normal_(self.shift_var3, 1, 0.1)
        self.shift_mean3 = nn.Parameter(torch.zeros(3, img_size - 6, img_size - 6))
        nn.init.normal_(self.shift_mean3, 0, 0.1)

        self.norm = nn.InstanceNorm2d(3)

        ############## Fixed Parameters (For MI estimation
        self.spatial = nn.Conv2d(3, 3, 3).cuda()
        self.spatial_up = nn.ConvTranspose2d(3, 3, 3).cuda()

        self.spatial2 = nn.Conv2d(3, 3, 5).cuda()
        self.spatial_up2 = nn.ConvTranspose2d(3, 3, 5).cuda()

        self.spatial3 = nn.Conv2d(3, 3, 7).cuda()
        self.spatial_up3 = nn.ConvTranspose2d(3, 3, 7).cuda()
        self.color = nn.Conv2d(3, 3, 1).cuda()
    ...
    def forward(self, x, estimation=False):
        if not estimation:
            spatial = nn.Conv2d(3, 3, 3).cuda()
            spatial_up = nn.ConvTranspose2d(3, 3, 3).cuda()

            spatial2 = nn.Conv2d(3, 3, 5).cuda()
            spatial_up2 = nn.ConvTranspose2d(3, 3, 5).cuda()

            spatial3 = nn.Conv2d(3, 3, 7).cuda()
            spatial_up3 = nn.ConvTranspose2d(3, 3, 7).cuda()
