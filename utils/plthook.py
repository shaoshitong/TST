...
class PltAboutHook(object):
    ...
    def __init__(
        self, model, type="forward", norm="mean", mode="multi", log=False, hook_function=None
    ):
        """
        Args:
            ...
            if self.norm == "mean":
                for i, input in enumerate(inputs):
                    self.buffer_dict[str(module) + str(id(module))]["inputs"][i].append(
                        (
                            self.count,
                            (input.norm() / input.numel()).detach().item()
                ...
                for i, output in enumerate(outputs):
                    self.buffer_dict[str(module) + str(id(module))]["outputs"][i].append(
                        (
                            self.count,
                            (output.norm() / output.numel()).detach().item()
        ...
        if self.mode == "multi":
            ...
            if scatter_y_tmp.shape[0] < scatter_y.shape[0]:
                ...
                for i, v in enumerate(scatter_y):
                    if math.isinf(v):
                        print(i, trun_name[i][: trun_name[i].rfind(")")])
            amin, amax = (
                scatter_y_tmp.mean() - scatter_y_tmp.std(),
                scatter_y_tmp.mean() + scatter_y_tmp.std(),
            )
            shown_x = []
            for x, y in zip(scatter_x, scatter_y):
                if y >= amax or y <= amin or math.isinf(y):
                    shown_x.append(x)
            shown_x = set(shown_x)
            print([cname if x in shown_x else "" for x, cname in zip(trun_x, cnames)])
            plt.xticks(
                trun_x,
                [cname if x in shown_x else "" for x, cname in zip(trun_x, cnames)],
                rotation=20,
            )
            plt.savefig(
                str(id(self.model))
