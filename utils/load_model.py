...
def load_model(model_dict,model):
    ...
    print(f"the prune number is {round((len(model_state_dict.keys())-len(pretrained_dict.keys()))*100/len(model_state_dict.keys()),3)}%")
...
def load_model_from_url(model,url):
    state_dict = torch.hub.load_state_dict_from_url(url, model_dir='/root/.cache/torch/hub/checkpoints',progress=True,map_location=torch.device('cpu'))
