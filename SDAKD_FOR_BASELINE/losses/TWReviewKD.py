...
class TWReviewKD(nn.Module):
    def __init__(self, shapes, out_shapes, in_channels, out_channels, max_mid_channel):
        ...
        for out_channel in out_channels:
            self.out_embeddings.append(
                nn.Sequential(
                    nn.Conv2d(out_channel, out_channel, (3, 3), (1, 1), (1, 1), bias=False),
                    nn.BatchNorm2d(out_channel),
                )
            )
    ...
    def forward(self, features_student, features_teacher):
        ...
        teacher_1 = (torch.stack(tmp_teacher_1, 0) * tmp_teacher_1_weight.view(-1, 1, 1, 1, 1)).sum(
            0
        )
        ...
        teacher_2 = (torch.stack(tmp_teacher_2, 0) * tmp_teacher_2_weight.view(-1, 1, 1, 1, 1)).sum(
            0
        )
        ...
        teacher_3 = (torch.stack(tmp_teacher_3, 0) * tmp_teacher_3_weight.view(-1, 1, 1, 1, 1)).sum(
            0
        )
        # losses
